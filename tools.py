...
@tool
async def get_upcoming_canvas_assignments() -> str:
    """Fetch un-synced upcoming assignments from Canvas LMS using Playwright.
    
    This command will spin up a headless browser, navigate to Canvas, and scrape assignments.
    ...
    """
    try:
        assignments = await canvas_sync.fetch_canvas_assignments(headless=True)
        if not assignments:
            return "No upcoming canvas assignments found."
        
        output = [f"Found {len(assignments)} Canvas Assignments:"]
        for a in assignments:
            output.append(f"- {a['title']} ({a['course_name']}) | Due: {a['due_date_str'] or 'None'} | Priority: {a['priority']} | Tags: {a['tags']}")
        return "\n".join(output)
    except Exception as e:
        return format_tool_error(e, "get_upcoming_canvas_assignments")

@tool
async def sync_canvas() -> str:
    """Synchronize Canvas assignments to TickTick.
    
    This tool scrapes Canvas for upcoming tasks and creates them in TickTick.
    It automatically prevents duplicates by checking existing tasks in the corresponding TickTick lists.

    Returns:
        Summary of tasks added to TickTick.
    """
    try:
        assignments = await canvas_sync.fetch_canvas_assignments(headless=True)
        if not assignments:
            return "No canvas assignments found to sync."
        ...
        for p_id in active_projects.keys():
             try:
        ...
        output_log = []
        for a in assignments:
            # 1. Determine the target project id
            target_project_id = a.get("ticktick_list_id", "")
            if not target_project_id:
                target_project_id = await get_inbox_id()
            ...
            if is_dup:
                # If it's a duplicate but the canvas assignment is submitted, complete the ticktick task
                if a.get('is_submitted'):
                    # Find the exact ticktick task to complete
                    try:
                        for et in active_projects.get(target_project_id, {}).get("tasks", []):
                             if a['title'] in et.title and et.status != 2: # 2 is Completed
                                 await _rate_limiter.acquire()
                                 await client.complete_task(target_project_id, et.id)
                                 output_log.append(f"Completed (Submitted in Canvas): {a['title']}")
                    except Exception as e:
                        output_log.append(f"Error completing {a['title']}: {e}")
                else:    
                    skipped_count += 1
                continue
                
            # If it's NOT a duplicate, but it's already submitted in Canvas, there is no need to create it 
            # as a new task in TickTick just to mark it complete. We just skip it entirely.
            if a.get('is_submitted'):
                skipped_count += 1
                continue
            ...
            try:
                await _rate_limiter.acquire()
                task_content = f"Source: {a['url']}\nCourse: {a['course_name']}"
                
                # We reuse the logic required by Dida365TaskCreate payload
                task_dict = {
                    "title": f"{a['title']} - {a['course_name']}",
                    "project_id": target_project_id,
                    "content": task_content,
                    "is_all_day": False
                }
                
                priority_map = {0: TaskPriority.NONE, 1: TaskPriority.LOW, 3: TaskPriority.MEDIUM, 5: TaskPriority.HIGH}
                task_dict["priority"] = priority_map.get(a['priority'], TaskPriority.NONE)
                if a['due_date']:
                ...
                added_count += 1
                output_log.append(f"Added: {a['title']}")
            except Exception as e:
                output_log.append(f"Error adding {a['title']}: {e}")

        summary = f"Sync Summary: {added_count} tasks added. {skipped_count} tasks skipped (duplicates).\n"
        if output_log:
             summary += "\nDetail:\n" + "\n".join(output_log)
        return summary
    except Exception as e:
